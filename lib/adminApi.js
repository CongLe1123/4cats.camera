...
function mapSupabaseToAdmin(cam) {
  const specs = (typeof cam.specs === "object" && cam.specs !== null) ? cam.specs : {};
  const variants = Array.isArray(specs.variants) && specs.variants.length > 0
    ? specs.variants.map((v, idx) => ({
        id: v.id || `var-${cam.id}-${idx}`,
        sku: v.sku || `SKU-${cam.id}-${idx + 1}`,
    ...
    : (cam.camera_variants || []).map((cv, idx) => ({
        id: cv.id || `var-${cam.id}-${idx}`,
        sku: cv.sku || `SKU-${cam.id}-${idx + 1}`,
        color: cv.color || cv.colors?.name || "Tiêu chuẩn",
  ...
  const minPrice = variants.length > 0 ? Math.min(...variants.map((v) => v.price).filter(p => p > 0)) : Number(cam.price) || 0;

  const brandName = cam.brand?.name || specs.brand || "Canon";
  const categoryName = cam.category?.name || specs.camera_type || "Mirrorless";
  const seriesName = cam.series?.name || specs.series || "";
  const slug = cam.slug || specs.slug || `camera-${cam.id}`;
  const mainImage = cam.image || specs.main_image || specs.image || "/favicon.ico";
  const galleryImages = Array.isArray(cam.images) && cam.images.length > 0 ? cam.images : (Array.isArray(specs.images) ? specs.images : [mainImage]);
  ...
  return {
    ...
    short_description: specs.short_description || cam.short_description || "",
    beginner_summary: specs.beginner_summary || cam.beginner_summary || "",
    ...
    content: cam.content || specs.content || "",
    variants,
    totalStock,
    minPrice: minPrice === Infinity ? 0 : minPrice,
    status: cam.status || specs.status || "published",
    is_published: cam.is_published !== false && specs.is_published !== false,
    is_featured: !!(cam.is_featured || specs.is_featured),
    featured_section: cam.featured_section || specs.featured_section || "",
    featured_order: cam.featured_order || specs.featured_order || 0,
    seo_title: specs.seo_title || `${cam.name} Chính Hãng — 4cats Camera`,
    seo_description: specs.seo_description || "",
    updated_at: cam.updated_at || cam.created_at || new Date().toISOString()
  };
}

// ----------------------------------------------------------------------
// 1. PRODUCTS & CATALOG (SUPABASE ONLY)
// ----------------------------------------------------------------------
...
export async function adminGetProducts(filters = {}) {
  ...
  if (error) {
    console.error("adminGetProducts error from Supabase:", error.message);
    throw new Error(`Không thể tải sản phẩm: ${error.message}`);
  }

  let products = (dbCameras || []).map(mapSupabaseToAdmin);
...
export async function adminGetProductById(id) {
  ...
  // 2. Try match by slug
  const { data: allCameras, error } = await supabase
    .from("cameras")
    .select(`
      id, name, image, images, content, features, specs, created_at,
      brand:brands(id, name),
  ...
  if (!error && allCameras) {
    for (const c of allCameras) {
      const mapped = mapSupabaseToAdmin(c);
      if (mapped.slug.toLowerCase() === strId.toLowerCase()) {
        return mapped;
      }
    }
  }
...
// ----------------------------------------------------------------------
// 3. HOMEPAGE & PROMOTION CAROUSEL (SUPABASE ONLY)
// ----------------------------------------------------------------------
...
export async function adminGetBanners() {
  ...
  if (error) {
    console.error("adminGetBanners error from Supabase:", error.message);
    return [];
  }
