...
export async function getCameraById(idOrSlug) {
  if (!idOrSlug) return null;
  const str = String(idOrSlug).trim();

  try {
    // 1. If numeric, attempt lookup by ID first
    ...
    // 2. Lookup by slug in Supabase (fetching all published cameras and matching slug)
    const { data, error } = await supabase
      .from("cameras")
      .select(CAMERA_SELECT_QUERY);

    if (error || !data) {
      console.error("Supabase getCameraById error:", error?.message);
      return null;
    }

    const lowerSlug = str.toLowerCase();
    for (const row of data) {
      const transformed = transformCamera(row);
      if (
        transformed.slug.toLowerCase() === lowerSlug ||
        String(transformed.id) === str
      ) {
        return transformed;
      }
    }
