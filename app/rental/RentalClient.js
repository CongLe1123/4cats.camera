...
function RentalContent({ cameras = [], filters = defaultFilters }) {
  const {
    brands = [],
    categories = [],
    conditions = [],
...
export default function RentalClient({ cameras, filters }) {
  return (
    <Suspense fallback={<div>Loading...</div>}>
      <RentalContent cameras={cameras} filters={filters} />
    </Suspense>
  );
}
